...
plot_outlier_barcode_over_alpha <- function(outnew) {
  ...
  dfl$y <- as.numeric(paste(dfl$y))
  x <- y <- strength <- NULL
  ggplot2::ggplot(dfl, ggplot2::aes(x = y, y = x, fill = strength)) +
    ggplot2::geom_raster() +
